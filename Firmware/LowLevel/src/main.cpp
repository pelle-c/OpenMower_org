...
// These are the absolute maximum voltage levels, for a 7S-pack with cells of 4.2V max the BATT_ABS_MAX will be 29.4V
...
// but a battery explosion could be serious. 
...
// Do we want to make an average of the values from the ADC?
#define AVERAGING_ADC_VALUES
...
float BATT_U_VALUES_ARR[10] = { 0 }; 
float CHARGE_U_VALUES_ARR[10] = { 0 }; 
float CHARGE_I_VALUES_ARR[10] = { 0 }; 
...
#ifdef USB_DEBUG
        DEBUG_SERIAL.println("Stop Charging");
#endif
...
#ifdef USB_DEBUG
        DEBUG_SERIAL.println("Start Charging");
#endif
                digitalWrite(PIN_ENABLE_CHARGE, HIGH);
...
// Shifts a new value into the first postition of an array
void shiftValueIntoArray(float *arr, float latest_value) {
...
// If averaging is configured(defined), then return...the average, otherwise the latest value
float getAverageOrLatest(float *arr) {
...
#ifdef AVERAGING_ADC_VALUES
    return float (0.2 * avg);
#else
    return float (arr[0]);
#endif
}
...
void loop() {
    ...
    if (now - last_status_update_millis > STATUS_CYCLETIME) {
        ...
        // Read the ADCs and update the averaging arrays
        shiftValueIntoArray(&BATT_U_VALUES_ARR[0], (float) analogRead(PIN_ANALOG_BATTERY_VOLTAGE) * (3.3f / 4096.0f) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_BATTERY_CORRECTION_FACTOR + ADC_BATTERY_CORRECTION_OFFSET);
        status_message.v_battery = (float) getAverageOrLatest(BATT_U_VALUES_ARR);

        shiftValueIntoArray(&CHARGE_U_VALUES_ARR[0], (float) analogRead(PIN_ANALOG_CHARGE_VOLTAGE) * (3.3f / 4096.0f) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_CHARGE_CORRECTION_FACTOR + ADC_CHARGE_CORRECTION_OFFSET);
        status_message.v_charge = (float) getAverageOrLatest(CHARGE_U_VALUES_ARR);

#ifndef IGNORE_CHARGING_CURRENT
        shiftValueIntoArray(&CHARGE_I_VALUES_ARR[0], (float) analogRead(PIN_ANALOG_CHARGE_CURRENT) * (3.3f / 4096.0f) / (CURRENT_SENSE_GAIN * R_SHUNT) * ADC_CHARGE_CURRENT_CORRECTION_FACTOR + ADC_CHARGE_CURRENT_CORRECTION_OFFSET);
        status_message.charging_current = (float) getAverageOrLatest(CHARGE_I_VALUES_ARR);                
        // The shunt that is used for current measurement is not involved when not charging. If the value is below zero, the om app shows "charging".
        if (status_message.charging_current < 0) {
